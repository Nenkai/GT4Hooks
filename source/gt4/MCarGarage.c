...
/* Hook: MCarGarage
   ...
   How: Redirect hModule::defineFunction(..., ..., "sortByDisplacement", ...) to our hook, 
        which will register sortByDisplacement on its own along with other functions.
...
void HOOK_ExtendMCarGarage(void* tempHValue, hModule* module, char* name, Adhoc_method_cb func)
{
...
{
    MCarGarage* garage;
    MCarGarage_MCarGarage(&garage, this);
